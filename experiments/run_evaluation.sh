...
parallel --bar --header : --results data/outdir_test_1200/outdir_simple_1200 -j 10 python src/base_opt/base_opt/evaluate_algorithm.py \
  ...
  --timeout 1200 ::: Space xyz xyz_rotvec ::: Alg BOOptimizer GAOptimizer RandomBaseOptimizer DummyOptimizer AdamOptimizer ::: Seed {1..5} > /dev/null
...
parallel --bar --header : --results data/outdir_test_1200/outdir_hard_1200 -j 40 python src/base_opt/base_opt/evaluate_algorithm.py \
  ...
  --solution-storage data/solutions/ \
  --timeout 1200 \
  --reward-fail -50 ::: Space xyz xyz_rotvec ::: Set test_hard test_realworld ::: Alg BOOptimizer GAOptimizer RandomBaseOptimizer DummyOptimizer AdamOptimizer ::: Seed {1..5} > /dev/null
...
parallel --bar --header : --results data/outdir_test_1200/outdir_edge_1200 -j 40 python src/base_opt/base_opt/evaluate_algorithm.py \
  ...
  --reward-fail -50 ::: Space xyz xyz_rotvec ::: Alg BOOptimizer GAOptimizer RandomBaseOptimizer DummyOptimizer AdamOptimizer ::: Seed {1..5} > /dev/null
